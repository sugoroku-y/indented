...
export function unescape(s: string): string {
  return s.replace(
    /\\(?:x(?<hex2>[0-9A-F]{2})|u(?<hex4>[0-9A-F]{4})|u\{0*?(?<hexCodePoint>[0-9A-F]{1,4}|(?:[1-9A-F]|10)[0-9A-F]{4})\}|0\d|\r\n|[^])/gi,
    //    ^^^^^^^^^^^^^^^^^^^^^                                                                                                      16進エスケープシーケンス
    //                          ^^^^^^^^^^^^^^^^^^^^^                                                                                Unicodeエスケープシーケンス
    //                                                ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^              Unicodeコードポイントエスケープ
...
function unsupportedEscapeSequence(index: number, input: string): void {
  const current = input.slice(index);
  // 不正なエスケープシーケンスのときなのでログを出す
  let match;
  console.error(
    (match = /^(?:\\x[^]{0,2}|\\u\{([^]*)\}|\\u[^]{0,4})/.exec(current)) != null
      ...
      : (match = /^\\(?:[0-3][0-7]{0,2}|[4-7][0-7]?)/.exec(current)) != null
        ? // 8進表記は使用できない
