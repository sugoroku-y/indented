...
function convert(raw) {
    ...
    if (!indent ||
        ...
        return raw.map(unescape_1.unescape);
    }
    return raw.map((s, i) => (0, unescape_1.unescape)(trimNewLine(unindent(s, indent), i, raw.length)));
}
/**
 * インデントを取り除く。
 * @param s 対象の文字列
 ...
 */
function unindent(s, indent) {
    return s.replace(UNINDENT_REGEX, (match, newline, whitespace) => whitespace.startsWith(indent)
 ...
 * @returns 改行を取り除いた文字列
 */
function trimNewLine(t, i, length) {
    // 引数を変更しないようにコピーを作成
    let s = t;
    if (i === 0) {
        // 先頭の改行を除去
